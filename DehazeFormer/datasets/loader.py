...
class PairLoader(Dataset):
	def __init__(self, data_dir, sub_dir, mode, size=256, edge_decay=0, only_h_flip=False):
		...
		print(data_dir, " ", sub_dir)
# 		self.root_dir = self.root_dir.split("/")[:-2]
# 		self.root_dir.append("DehazeFormer")
# 		self.root_dir = "/".join(self.root_dir) 
# 		self.root_dir = os.path.join(self.root_dir, sub_dir)
# 		print(self.root_dir)
	...
	def __getitem__(self, idx):
		...
		# print(os.path.join(self.root_dir, 'hazy', img_name))
...
class SingleLoader(Dataset):
	...
	def __getitem__(self, idx):
		...
		# read image, and scale [0, 1] to [-1, 1]
		img_name = self.img_names[idx]
		# print(os.path.join(self.root_dir, img_name))
