...
def populate_train_list(orig_images_path, hazy_images_path): 
  ...
  for image in image_list_haze:
    image = image.split("/")[-1]
    images.append(image)
		#key = image.split("  ")[0] + "  " + image.split("  ")[1] + ".JPG"
  ...
  train_keys = images[0:150]
  val_keys = images[151::]

  for key in train_keys:
    train_list.append([orig_images_path + key])

  for key in val_keys:
    val_list.append([hazy_images_path + key])
...
class dehazing_loader(data.Dataset):
    ...
    #print(self.data_list[index])
		...
		data_orig_path, data_hazy_path = self.data_list[index], self.data_list[index]
